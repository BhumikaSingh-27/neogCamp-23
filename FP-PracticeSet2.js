...
const multiplyByFive = numbers.map( ele => ele*5)
...
const greetings = ({name, dept}) => `Hi ${name} we are glad to have you as a ${dept}`
const greetEmployeeMessages = employeeData.map(greetings)
...
const books = [
  ...
  const getBookTitles = (books) => books.map((obj)=> obj.title)
...
const studentNames = studentName.filter( str => str[0] === 'A')
...
const students = [
  ...
  const findStudentWithGradeA = (students) => students.filter(({grade})=> grade==='A')
  ...
  const findStudent = students => students.find(({grade, scholarship}) => grade === 'A' ||scholarship === true )
...
const findStudent = (students) => students.find(({grade,scholarship}) => grade === 'B' && scholarship === true)
  const student = findStudent(students);
  console.log(student); 
  ...
  const getBestOldBollywoodMovies = (bollywoodMovies) => {const movieObject = bollywoodMovies.filter(({year, rating}) => year <1990 && rating > 8.0);
  const titleOfMovie = movieObject.map((movie)=> movie.title)
  return titleOfMovie
