...
export const Ques6 = () => {
  ...
  const showArchive = () => {
    setHeading("Archived");
    console.log("inside funcr", taskData);
  ...
  return (
    <>
      ...
      <ul>
        {taskData.map(
          ({ title, desc, daysFollowed, daysSkipped, archived }, index) => ( !archived &&
            <li key={index}>
              <h2>{title}</h2>
              {desc}
              <p>DayFollowed:{daysFollowed}</p>
      ...
      <ul>
        {heading === "Archived" &&
          taskData.map(({ title, desc, daysFollowed, daysSkipped }, index) => (
            <li key={index}>
              <h2>{title}</h2>
              {desc}
              <p>DayFollowed:{daysFollowed}</p>
